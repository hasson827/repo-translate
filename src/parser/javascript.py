...
class JavaScriptParser(Parser):
    ...
    def _extract_comments(self, node, result: ParseResult) -> None:
        if node.type == "comment":
            ...
            if comment_text.startswith("/**"):
                comment_type = CommentType.JSDOC
    ...
    def _clean_comment_text(self, comment: str, comment_type: CommentType) -> str:
        if comment_type == CommentType.SINGLE_LINE:
            ...
            for line in text.split("\n"):
                ...
                lines.append(line)
            return "\n".join(lines).strip()
        elif comment_type == CommentType.JSDOC:
            text = comment.strip("/*").strip("*/").strip()
            lines = []
            for line in text.split("\n"):
                line = line.strip()
    ...
    def inject_translations(
        ...
        for idx in sorted(translations.keys(), reverse=True):
            ...
            if comment.type == CommentType.SINGLE_LINE:
            ...
            elif comment.type in (CommentType.MULTI_LINE, CommentType.JSDOC):
                is_jsdoc = comment.type == CommentType.JSDOC
                start_marker = "/**" if is_jsdoc else "/*"
                ...
                if len(trans_lines) == 1:
                    end_marker = " */" if is_jsdoc else "*/"
                    lines[start_line] = f"{indent}{start_marker} {translated}{end_marker}"
                else:
                    new_lines = [f"{indent}{start_marker}"]
...
class TypeScriptParser(JavaScriptParser):
    ...
    def _get_language(self) -> Language:
        return Language(tstype.language_typescript())
