...
try:
    ...
    HAS_TREE_SITTER_SWIFT = True
except ImportError:
    HAS_TREE_SITTER_SWIFT = False
    tsswift = None
    TSParser = None
    Language = None
...
class SwiftParser(Parser):
    ...
    def _extract_comments(self, node, result: ParseResult) -> None:
        if node.type == "comment" or node.type == "line_comment" or node.type == "block_comment":
